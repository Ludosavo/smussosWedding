...
// Configuration
const CONFIG = {
  inputDir: path.join(__dirname, '../src/components/pictures'),
  outputDir: path.join(__dirname, '../src/components/pictures/optimized'),
  sizes: [
    ...
    { suffix: 'full', width: 1920, quality: 90 }
  ],
...
async function optimizeImage(sharp, inputPath, outputDir, filename) {
  const baseName = path.parse(filename).name.toLowerCase().replace(/\s+/g, '-')
...
function generateImportsFile(processedImages, outputDir) {
  ...
  processedImages.forEach(img => {
    const varName = img.baseName.replace(/-/g, '_')
    
    CONFIG.sizes.forEach(size => {
      const importName = `${varName}_${size.suffix}`
      const filename = `${img.baseName}-${size.suffix}.webp`
      imports.push(`import ${importName} from './optimized/${filename}'`)
    })
    
    // Create an object export for each image with all sizes
    const sizeExports = CONFIG.sizes.map(size => 
      `  ${size.suffix}: ${varName}_${size.suffix}`
    ).join(',\n')
    
    exports.push(`export const ${varName} = {\n${sizeExports}\n}`)
  })
...
`
  ...
  console.log(`\n📝 Generated imports file: src/components/pictures/optimized-images.js`)
}
